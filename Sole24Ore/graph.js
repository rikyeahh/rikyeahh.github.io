...
d3.json("complete_processed.json").then(function (data) {
    ...
    function buildImageTag(imagePath) {
        maxW = 100;
        maxH = 100;
        return `<img src="${imagePath}" style="max-width: ${maxW}px; max-height: ${maxH}px;">`
    }
    data.forEach(function (d) {
        ...
        svg.append('circle')
            ...
            .attr('fill', categoryToColor(category))
            .attr('opacity', 0.66)
            .attr('textOnHover', `<b>${d.name}</b> (${d.age} years)
                <br>${buildImageTag(d.img_local_path)}
                <br><hr> Awarded in ${d.awardYear} ${d.motivation}.`)
